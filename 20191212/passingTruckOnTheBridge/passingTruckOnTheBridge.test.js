...
const getMinSecond = (length, limit, truck_weights) => {
    ...
    while(bridge.length !== 0 || truck_weights.length !== 0){
        if (getWeightsOnTheBridge(bridge) + truck_weights[count] <= limit) {
...
test('getMinSecond', () => {
    ...
    // expect(getMinSecond(2,7,[7,4])).toBe(4);
    // expect(getMinSecond(2,7,[7,4,5,6])).toBe(6);
